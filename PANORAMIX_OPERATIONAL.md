...
Ctl exports: `GET /v0/jobs/{id}/handoff` (WorkHandoff projection) and `GET /v0/jobs/{id}/payload` (canonical bytes when a demo stored them). Guest does not open mesh ctl. `runtime.apply` is used only when the operator set `PANORAMIX_RUNTIME_ROOT` (G13 `dsl progress --id`; G15 `dsl admit --live` for matching R2 catalogs).
...
| Call `runtime.apply` or open guest→ctl mesh HTTP from this Unit | **Rejected as mesh.** G13/G15 may invoke `python3 -m runtime.apply` only when the operator set `PANORAMIX_RUNTIME_ROOT` (progress / live admit). No guest→mesh ctl hop. Missing engine env keeps the digest stub |
...
| Stamp north-star Done / unlock cloud from stub jobs, a catalog API, the editor, files browse, runs UX, the G7 probe, G8 chat, G10 React Flow, G9 Matryoshka, G11 catalogs, G12 submit dialog, G13 durable progress, or G15 live admit | **Rejected** — G1–G15 are seams / polish; epic [#1](https://github.com/guypayeur/panoramix-guest-dsl/issues/1) remains open; cloud stays locked |
