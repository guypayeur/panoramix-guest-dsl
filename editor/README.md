...
Auto-layout is **Sugiyama / ELK Layered** (`elkjs`, `elk.algorithm: layered`, `elk.hierarchyHandling: INCLUDE_CHILDREN`) so catalog graphs (SOS / RESERVE / sos-lite) get a left-to-right dataflow, not type-column piles. Child coordinates stay parent-relative. If elkjs fails to load, a deterministic edge-aware layered fallback still ranks by topology (cycle break → longest-path layers → barycenter order). Opening a catalog spec and the Auto-layout button both run this layout, then `fitView`.
