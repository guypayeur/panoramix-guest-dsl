...
Auto-layout is **Sugiyama / ELK Layered** (`elkjs`, `elk.algorithm: layered`, `elk.hierarchyHandling: INCLUDE_CHILDREN`) so catalog graphs (SOS / RESERVE / sos-lite) get a left-to-right dataflow, not type-column piles. Child coordinates stay parent-relative. Leaf sizes come from **measured / content-estimated height** (formula cards grow with their key list) plus DOM `getBoundingClientRect` after paint — not a fixed 88px box. If elkjs fails to load, a deterministic edge-aware layered fallback still ranks by topology (cycle break → longest-path layers → barycenter order). Opening a catalog spec and the Auto-layout button both run this layout, then `fitView`. Expand/collapse re-packs the compound.
