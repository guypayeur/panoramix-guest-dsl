...
class EditorLoadTests(unittest.TestCase):
    ...
    def test_editor_html_and_assets(self) -> None:
        for path in ("/", "/ui", "/ui/", "/ui/index.html"):
            ...
            self.assertIn("data-testid=\"auto-refresh\"", html)
            self.assertIn("data-testid=\"run-progress\"", html)
            ...
            self.assertIn("data-testid=\"scope-crumbs\"", html)
            self.assertIn("data-testid=\"matryoshka\"", html)
